...
if [ -f package.json ]; then
  ...
  if python3 -m pytest --collect-only -q >/dev/null 2>&1; then TEST="python3 -m pytest -q"; else TEST=""; fi  # collect-only: exit 0 only if >=1 test; no tests => TEST="" => exit 4 (not a false exit 3)
elif [ -f pom.xml ]; then
